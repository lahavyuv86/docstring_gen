...
def generate_docstring_with_ollama(code_snippet: str, config: dict, model: str) -> str:
    prompt = (
        ...
        "1. A short description, should be in imperative mood (e.g., 'Create' instead of 'Creates').\n"
        "2. 'Args:' section (only if the function has parameters) with parameter names, types, and descriptions.\n"
        "3. 'Returns:' section (only if the function has a return value) with the return type and description.\n"
        "4. Keep the description and the Args and Returns sections at the same indentation.\n"
        "5. Keep the bodies inside the Args and Returns sections at 1 indentation inner (4 spaces).\n\n"
        "Have spaces between each section in the docstring.\n\n"
        "Do not include 'Args:' or 'Return:' sections in the docstring when they are 'Args: None' or 'Returns: None'.\n\n"
